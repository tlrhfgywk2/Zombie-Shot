...
describe('수치 방어층과 방어 파괴', () => {
  it('방어 24 표적은 방어 파괴 12 철갑탄 한 발에 방어가 12만 감소한다', () => {
    ...
    expect(shot.breakdown.armorBlocked).toBe(12);
    expect(shot.armorDamage).toBe(12);
    expect(shot.after.armor).toBe(12);
  ...
  it('큰 방어층은 흡수한 피해만 감소한다', () => {
    ...
    expect(shot.breakdown.armorBlocked).toBe(22);
    expect(shot.armorDamage).toBe(22);
