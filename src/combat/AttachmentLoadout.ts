import { ATTACHMENT_DEFINITIONS, ATTACHMENT_SLOT_ORDER, DEFAULT_LOADOUT, type AttachmentId, type LoadoutSnapshot } from '../data/attachmentDefinitions';
...
export const getEnabledAttachmentIds = (loadout: LoadoutSnapshot, playerState: PlayerCombatState = createPlayerCombatState()): AttachmentId[] => ATTACHMENT_SLOT_ORDER.flatMap((slot) => {
  ...
  return id && !playerState.disabledSlots[slot] ? [id] : [];
});
...
export class AttachmentLoadout {
  ...
  equip(id: AttachmentId): AttachmentId | undefined { const definition = ATTACHMENT_DEFINITIONS[id]; const replaced = this.equipped[definition.slot]; this.equipped[definition.slot] = id; return replaced; }
