...
describe('인접 탄약 효과', () => {
  it.each([
    ...
    expect(resolver.resolveSequence(rounds, target()).shots.map(shot => shot.breakdown.effectiveFirepower)).toEqual(expected);
  });
  it('강한 반동이 방어 파괴와 충격을 깎지 않는다', () => {
    ...
    expect(armor.shots[1]?.breakdown.effectiveFirepower).toBe(1);
    const shock = resolver.resolveSequence(['overpressure', 'flatPoint'], target());
    expect(shock.shots[1]?.actionShockApplied).toBe(5);
    expect(shock.shots[1]?.breakdown.effectiveFirepower).toBe(2);
  });
  it('안정은 반동을 소비하고 두 순서 효과는 다음 탄창으로 넘어가지 않는다', () => {
    ...
    for (const last of ['overpressure', 'flatPoint'] as const) {
      ...
      expect(next.shots[0]?.breakdown).toMatchObject({ effectiveFirepower: 3, projectedShock: 2 });
    }
  });
  it.each([
  ...
  it.each([['hollowPoint', 1, 4], ['hollowPoint', 0, 7], ['bonded', 5, 5], ['bonded', 0, 3]] as const)('%s 장갑 %s 사격 시작 조건을 검사한다', (ammo, armor, firepower) => {
    ...
    expect(shot.breakdown.effectiveFirepower).toBe(firepower);
    expect(shot.after.armor).toBe(0);
    if (ammo === 'bonded' && armor > 0) expect(shot.breakdown.armorBroken).toBe(5);
  });
  it('매치탄은 반동으로 화력이 줄어도 발사 순서 전체의 3%p 완화를 유지한다', () => {
    const single = resolver.resolveShot('match', 0, target(0, 11));
    expect(single.breakdown.effectiveFirepower).toBe(3);
    const unstable = resolver.resolveSequence(['overpressure', 'match'], target(0, 11));
    expect(unstable.shots[1]?.breakdown.effectiveFirepower).toBe(1);
    expect(unstable.finalRangePenaltyPercent).toBe(22);
    const stable = resolver.resolveSequence(['overpressure', 'subsonic', 'match'], target(0, 11));
    expect(stable.shots[2]?.breakdown.effectiveFirepower).toBe(3);
    expect(stable.finalRangePenaltyPercent).toBe(22);
  });
  it('지반 충격은 2턴 동안 반동 후속 화력만 -3으로 바꾼다', () => {
    ...
    expect(sequence.shots[1]?.breakdown).toMatchObject({ heavyKickPenalty: 3, effectiveFirepower: 1, projectedShock: 5 });
    const stable = resolver.resolveSequence(['overpressure', 'subsonic'], target(), { playerState: state });
    expect(stable.shots[1]?.breakdown.effectiveFirepower).toBe(4);
