...
const normal = (title: string, roster: readonly EnemyType[]): RouteOption => ({
  kind: 'normal', title, subtitle: '예측 가능한 감염체 무리', roster, reward: '탄약 또는 휴대 용량 강화 · 다음 구간 잔량 회복',
});
const special = (enemy: EnemyType): RouteOption => ({
  ...
  roster: [enemy], reward: '미소유 부착물 1개 확정 · 탄약 또는 휴대 용량 강화',
});
export const ENCOUNTER_STAGES: readonly EncounterStage[] = [
  ...
  { normal: normal('무너진 검문소', ['brute', 'normal']), special: special('groundshaker') },
  { normal: normal('공명 지하도', ['fast', 'brute']), special: special('screecher') },
  { normal: normal('최종 방어선', ['tough', 'fast', 'brute']), special: special('groundshaker') },
];
