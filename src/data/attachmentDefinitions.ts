...
export const ATTACHMENT_DEFINITIONS: Record<AttachmentId, AttachmentDefinition> = {
  returnBrake: {
    id: 'returnBrake', name: '회귀 제동기', slot: 'muzzle', summary: '연속 사격 반동 감소', tradeoff: '원거리 정확도 -8%',
    modifiers: [{ kind: 'recoilStep', value: -4 }, { kind: 'accuracy', value: -8, condition: { range: 'far' } }],
  },
  quietBore: {
  ...
  reflexSight: {
    id: 'reflexSight', name: '반사 조준기', slot: 'optic', summary: '근·중거리 정확도 +12%', tradeoff: '원거리 정확도 -6%',
    modifiers: [{ kind: 'accuracy', value: 12, condition: { range: 'near' } }, { kind: 'accuracy', value: 12, condition: { range: 'mid' } }, { kind: 'accuracy', value: -6, condition: { range: 'far' } }],
  },
  rangeSight: {
    id: 'rangeSight', name: '거리계 조준경', slot: 'optic', summary: '원거리 정확도 +22%', tradeoff: '근거리 정확도 -8%',
    modifiers: [{ kind: 'accuracy', value: 22, condition: { range: 'far' } }, { kind: 'accuracy', value: -8, condition: { range: 'near' } }],
  },
  closeLaser: {
  ...
  weightedGrip: {
    id: 'weightedGrip', name: '중량 손잡이', slot: 'grip', summary: '탄당 반동 손실 -5%', tradeoff: '첫 탄 정확도 -6%',
    modifiers: [{ kind: 'recoilStep', value: -5 }, { kind: 'accuracy', value: -6, condition: { firstShot: true } }],
  },
  etchedGrip: {
